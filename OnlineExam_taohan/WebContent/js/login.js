...
$(function() {
	...
	$("#signSubmit").click(function() {
		...
		if(studentAccount.trim() == null || studentAccount.trim() == "" || studentAccount.length <= 0 || studentAccount.length > 10) {
			zeroModal.show({
				title: "提示",
				content: "考试登录账户不合法!",
				width : '200px',
		...
		if(studentPwd.trim() == null || studentPwd.trim() == "" || studentPwd.length < 6 || studentPwd.length > 10) {
			zeroModal.show({
				title: "提示",
				content: "考试登录密码不合法!",
				width : '200px',
...
function receptionLoginValidate() {
	...
	if(studnetAccount.trim() == "" || studnetAccount == null) {
		loginTrue = false;
		$("#studentAccountMsg").text("请输入账号");
		return false;
	}
	var studentPwd = $("#inputPassword3").val();
	if(studentPwd == null || studentPwd.trim() == "") {
		loginTrue = false;
		$("#studentPwdMsg").text("请输入密码");
		return false;
	}
